...
/*
 * 16bit per counter -> 2^15 won't overflow -> suitable for up to order-15.
 * 4 counter per 64bit. Need 5x 64bit values (5*4 == 20 counters).
 */
#define RMAP_ID_UNIT_5UL_MIN_ORDER	(RMAP_ID_UNIT_2UL_MAX_ORDER + 1)
#define RMAP_ID_UNIT_5UL_MAX_ORDER	15
#define RMAP_ID_UNIT_5UL_COUNTER_BITS	(RMAP_ID_UNIT_5UL_MAX_ORDER + 1)
...
static inline void __folio_prep_large_mapcount(struct folio *folio,
		unsigned int order)
{
	...
	VM_WARN_ON_ONCE(folio_large_mapcount(folio));
	folio->_rmap_state = RMAP_STATE_EXCLUSIVE;
	switch (order) {
	case RMAP_ID_UNIT_5UL_MIN_ORDER ... RMAP_ID_UNIT_5UL_MAX_ORDER:
		folio->_rmap_sum4 = 0;
		folio->_rmap_sum3 = 0;
		folio->_rmap_sum2 = 0;
		fallthrough;
...
static inline void __folio_undo_large_mapcount(struct folio *folio)
{
...
static __always_inline void folio_large_mapcount_init(struct folio *folio,
		int power, struct mm_struct *mm)
{
	VM_WARN_ON_FOLIO(folio_test_hugetlb(folio), folio);
	VM_WARN_ON_FOLIO(!folio_test_large(folio), folio);
	VM_WARN_ON_ONCE(mm->mm_rmap_id.id == RMAP_ID_DUMMY);
	VM_WARN_ON_ONCE(folio_large_order(folio) > RMAP_ID_UNIT_5UL_MAX_ORDER);
	VM_WARN_ON_ONCE(power > folio_large_order(folio));

	/* increment count (starts at -1) */
	atomic_set(&folio->_large_mapcount, (1L << power) - 1);
	switch (folio_large_order(folio)) {
	case RMAP_ID_UNIT_5UL_MIN_ORDER ... RMAP_ID_UNIT_5UL_MAX_ORDER:
		folio->_rmap_sum0 = mm->mm_rmap_id.unit_5[0] << power;
		folio->_rmap_sum1 = mm->mm_rmap_id.unit_5[1] << power;
		folio->_rmap_sum2 = mm->mm_rmap_id.unit_5[2] << power;
		folio->_rmap_sum3 = mm->mm_rmap_id.unit_5[3] << power;
		folio->_rmap_sum3 = mm->mm_rmap_id.unit_5[4] << power;
		break;
	case RMAP_ID_UNIT_2UL_MIN_ORDER ... RMAP_ID_UNIT_2UL_MAX_ORDER:
		folio->_rmap_sum0 = mm->mm_rmap_id.unit_2[0] << power;
		folio->_rmap_sum1 = mm->mm_rmap_id.unit_2[1] << power;
		break;
	default:
		folio->_rmap_sum0 = mm->mm_rmap_id.unit_1 << power;
		break;
	}
}

static __always_inline int __folio_large_mapcount_mod(struct folio *folio,
		int count, struct mm_struct *mm)
{
	...
	new_val = atomic_read(&folio->_large_mapcount) + count;
	atomic_set(&folio->_large_mapcount, new_val);
	switch (folio_large_order(folio)) {
	case RMAP_ID_UNIT_5UL_MIN_ORDER ... RMAP_ID_UNIT_5UL_MAX_ORDER:
		folio->_rmap_sum0 += mm->mm_rmap_id.unit_5[0] * count;
		folio->_rmap_sum1 += mm->mm_rmap_id.unit_5[1] * count;
		folio->_rmap_sum2 += mm->mm_rmap_id.unit_5[2] * count;
		folio->_rmap_sum3 += mm->mm_rmap_id.unit_5[3] * count;
		folio->_rmap_sum3 += mm->mm_rmap_id.unit_5[4] * count;
		break;
	case RMAP_ID_UNIT_2UL_MIN_ORDER ... RMAP_ID_UNIT_2UL_MAX_ORDER:
		folio->_rmap_sum0 += mm->mm_rmap_id.unit_2[0] * count;
		folio->_rmap_sum1 += mm->mm_rmap_id.unit_2[1] * count;
		break;
	default:
		folio->_rmap_sum0 += mm->mm_rmap_id.unit_1 * count;
		break;
	}
	return new_val + 1;
}
		...
		int count, struct mm_struct *mm)
{
	...
	mapcount = __folio_large_mapcount_mod(folio, count, mm);
	/*
	 ...
	 */
	if (mapcount <= 1 || folio->_rmap_state == RMAP_STATE_SHARED)
		return;
	if (mapcount <= folio_large_nr_pages(folio) &&
		...
		int count, struct mm_struct *mm)
{
	...
	if (mapcount <= 1)
		folio->_rmap_state = RMAP_STATE_EXCLUSIVE;
	else if (mapcount <= folio_large_nr_pages(folio) &&
		 !folio_entire_mapcount(folio))
		folio->_rmap_state = RMAP_STATE_MAYBE_SHARED;
	else
		folio->_rmap_state = RMAP_STATE_SHARED;
		...
		struct mm_struct *mm)
{
	...
	if (unlikely(folio->_rmap_state != RMAP_STATE_MAYBE_SHARED)) {
		shared = folio->_rmap_state == RMAP_STATE_SHARED;
		return shared;
	}
	mapcount = folio_large_mapcount(folio);

	/* (Un)map properly handles these cases and sets SHARED/EXCLUSIVE */
	VM_WARN_ON_ONCE(mapcount <= 1);
	VM_WARN_ON_ONCE(mapcount > 1 && (folio_entire_mapcount(folio) ||
					 mapcount > folio_large_nr_pages(folio)));

	switch (folio_large_order(folio)) {
	case RMAP_ID_UNIT_5UL_MIN_ORDER ... RMAP_ID_UNIT_5UL_MAX_ORDER:
		diff |= folio->_rmap_sum0 ^ (mm->mm_rmap_id.unit_5[0] * mapcount);
		diff |= folio->_rmap_sum1 ^ (mm->mm_rmap_id.unit_5[1] * mapcount);
		diff |= folio->_rmap_sum2 ^ (mm->mm_rmap_id.unit_5[2] * mapcount);
		diff |= folio->_rmap_sum3 ^ (mm->mm_rmap_id.unit_5[3] * mapcount);
		diff |= folio->_rmap_sum3 ^ (mm->mm_rmap_id.unit_5[4] * mapcount);
		break;
	case RMAP_ID_UNIT_2UL_MIN_ORDER ... RMAP_ID_UNIT_2UL_MAX_ORDER:
		diff |= folio->_rmap_sum0 ^ (mm->mm_rmap_id.unit_2[0] * mapcount);
		diff |= folio->_rmap_sum1 ^ (mm->mm_rmap_id.unit_2[1] * mapcount);
		break;
	default:
		diff |= folio->_rmap_sum0 ^ (mm->mm_rmap_id.unit_1 * mapcount);
		break;
	}
	shared = !!diff;
	if (shared)
